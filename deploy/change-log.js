...
const pattern =
  /(\[(FEATURE|BUGFIX|DOCS|STYLE|REFACTO|TEST|QUALITY|CHORE|RELEASE)\](\[(INN|HMP|PAY|AUT|TMS|CRM|GED)-(\d)+\])? [A-Z]).*/g;
...
const generateDiff = (targetTag = null, beforeTag = null, version = null) => {
  ...
  exec(`git log --no-merges ${interval} --pretty=oneline`, (error, stdout, stderr) => {
    if (error) {
      console.error(stderr);
      throw error;
    }
...
gittags.get((err, tags) => {
  if (err) throw err;
  exec('git checkout origin/main', (error2, stdout2, stderr2) => {
    if (error2) {
    ...
    let lastTag = null;
    console.info(tags, version);
    if (tags && tags.length >= 1 && version) {
      [lastTag] = tags;
      if (lastTag === `v${version}`) {
        throw new Error('version not good');
      }
      exec(`git rev-list -n 1 ${lastTag}`, (error1, stdout1, stderr1) => {
        if (error1 || stderr1) throw stderr1;
