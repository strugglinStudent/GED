...
const theming = plugin.withOptions((options) => ({
                                                 ...
                                                 }) =>
  {
    ...
    /* Get the file path */
    const filename = path.resolve(__dirname, ('../../../../assets/styles/user-themes.scss'));
    ...
    addComponents(
      _.fromPairs(_.map(options.themes, (theme, themeName) => [
        ...
        ))), (value, key) => [[`--ged-${e(key)}`, value], [`--ged-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
      ]))
    );
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
    {
      ...
      const background = theme(`ged.customProps.background.${colorScheme}`);
      const foreground = theme(`ged.customProps.foreground.${colorScheme}`);
      ...
      return {
        [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
          ...
          ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--ged-${e(key)}`, value], [`--ged-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
          ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--ged-${e(key)}`, value], [`--ged-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        }
      };
    });
      ...
      theme: {
        extend: {
          ...
          colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
            [name, `rgba(var(--ged-${name}-rgb), <alpha-value>)`],
            [`on-${name}`, `rgba(var(--ged-on-${name}-rgb), <alpha-value>)`]
          ])))
        },
        ged  : {
          customProps: {
