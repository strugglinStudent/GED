...
import { NgbModule, NgbAlertModule, NgbAlert, NgbCollapse } from '@ng-bootstrap/ng-bootstrap';
...
const SUCCESS_STATUSES = [new FileStatus(), new FileStatus(), new FileStatus(), new FileStatus()];
...
describe('UploadFileWidgetComponent', () => {
  ...
  beforeEach(async () => {
    TestBed.configureTestingModule({
      declarations: [UploadFileWidgetComponent, WidgetFrameComponent, IfPermissionsDirective],
      providers: [
        PermissionsGuard,
        {
  ...
  it('should generate stats summary', () => {
    ...
    expect(component.getStatusSummary()).toEqual('Processing: 6, Failed: 1, Added: 4');
  });
  ...
  it('should enforce a maximum number of alerts', () => {
    ...
    fixture.detectChanges();
    // 5 total, 1 hidden
    expect(fixture.debugElement.queryAll(By.directive(NgbAlert))).toHaveLength(6);
    expect(
      fixture.debugElement.query(By.directive(NgbCollapse)).queryAll(By.directive(NgbAlert)),
    ).toHaveLength(1);
  });
  ...
  it('should allow dismissing completed alerts', fakeAsync(() => {
    ...
    fixture.detectChanges();
    jest.spyOn(component, 'getStatusCompleted').mockImplementation(() => SUCCESS_STATUSES);
    const dismissSpy = jest.spyOn(consumerStatusService, 'dismiss');
...
function mockConsumerStatuses(consumerStatusService) {
  ...
  jest.spyOn(consumerStatusService, 'getConsumerStatus').mockImplementation((phase) => {
    switch (phase) {
  ...
  jest.spyOn(consumerStatusService, 'getConsumerStatusNotCompleted').mockImplementation(() => {
    return DEFAULT_STATUSES;
  });
}
